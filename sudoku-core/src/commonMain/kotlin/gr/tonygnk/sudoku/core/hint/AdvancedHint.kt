...
class AdvancedHint(
            ...
            for (j in board.indices) {
                if (board[i][j].value != 0 && board[i][j].value != solvedBoard[i][j].value) {
                    return AdvancedHintData(
                        titleKey = "hint_wrong_value_title",
                        textKeyWithArg = Pair(
                            "hint_wrong_value_detail",
                            listOf(
    ...
    private fun checkForNakedSingle(): AdvancedHintData? {
        ...
        return if (!singles.isNullOrEmpty()) {
            ...
            return AdvancedHintData(
                titleKey = "hint_naked_single_title",
                textKeyWithArg = Pair(
                    "hint_naked_single_detail",
                    listOf(
        ...
        for (group in entity) {
            if (group.count { it.value != 0 } == type.size - 1) {
                ...
                if (emptyCell != null) {
                    ...
                    return AdvancedHintData(
                        titleKey = "hint_full_house_group_title",
                        textKeyWithArg = Pair(
                            "hint_full_house_group_detail",
                            listOf(
    ...
    private fun checkForHiddenSingle(): AdvancedHintData? {
        ...
        return AdvancedHintData(
            titleKey = "hint_hidden_single_title",
            textKeyWithArg = Pair(
                "hint_hidden_single_detail",
                listOf(
                    cellStringFormat(cell),
                    cell.value.toString()
                )
